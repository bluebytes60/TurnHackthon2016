...
function setValues(){
var current = readJsonFile("./status/status");
id = current["0"].id;
...
;( function( window ) {
	...
	grid3D.prototype._initEvents = function() {
		...
		this.gridItems.forEach( function( item, idx ) {setInterval(function () {
			setValues();
        if(home == 0 && id >=0) {
			self._showContent( id );
			setInterval(tryIt, 2000);
        }
		else if(home == 1 && id >=0)
		{
			//self._hideContent( id );
			autoRefresh1();
		}
		else
		{
